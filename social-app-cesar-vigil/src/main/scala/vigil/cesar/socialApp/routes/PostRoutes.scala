...
import vigil.cesar.socialApp.Main.{IMAGE_FOLDER, postManagerActor, system, userRegistrationActor}
...
object PostRoutes extends BasicRoute{
  ...
  def postRoutes(): Route =
    ...
    pathPrefix("posts") {
      ...
      (path("create" / IntNumber) | parameter('userId.as[Int])){ userId =>
        (post & toStrictEntity(3.seconds)) {
      ...
      (path( IntNumber / IntNumber) & get) { (sortOrderAsc, userId) =>
